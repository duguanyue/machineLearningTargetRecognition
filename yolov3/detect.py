...
def arg_parse():
    ...
    parser.add_argument(
        ...
        dest="images",
        help="待检测图像目录",
        default="path/to your sample",
        type=str,
    )
    parser.add_argument(
        "--det", dest="det", help="检测结果保存目录", default="path/to your save dir", type=str
    )
    parser.add_argument("--bs", dest="bs", help="Batch size，默认为1", default=1)
    parser.add_argument(
        "--confidence", dest="confidence", help="目标检测结果置信度阈值", default=0.5
    )
    parser.add_argument(
        "--nms_thresh", dest="nms_thresh", help="NMS非极大值抑制阈值", default=0.4
    )
    parser.add_argument(
        "--cfg", dest="cfgfile", help="配置文件", default="cfg/yolov3.cfg", type=str
    )
    parser.add_argument(
        "--weights", dest="weightsfile", help="模型权重", default="yolov3.weights", type=str
    )
    parser.add_argument(
        "--reso", dest="reso", help="网络输入分辨率.分辨率越高,则准确率越高;反之亦然", default="416", type=str
    )
